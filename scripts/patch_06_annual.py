...
MARKER = "HUB_ANNUAL_RETURNS_V13"

JS = r"""
/* HUB_ANNUAL_RETURNS_V13：年度回报区块（组合总览／逐只基金，跟随 06 的切换钮与语言） */
(function(){
  ...
  /* 列印模板的軸標籤問題：最大值／基準線／最小值三個標籤是各自算 y 的，
     當最小值接近 0（或最小值為 0）時會與「0%」基準標籤畫在幾乎同一位置 →
     印出來疊字（看起來又粗又大）。規則：同一位置（<12px）的標籤只留一個，
     優先保留「0%」基準標籤，否則保留最上面那個。 */
  function numOf(t){
  ...
  function fixAxisLabels(doc){
    Array.prototype.forEach.call(doc.querySelectorAll('article.chart-card svg'), function(svg){
      ...
      labs.forEach(function(t){
        ...
        var near = kept.filter(function(k){ return Math.abs(parseFloat(k.getAttribute('y')) - y) < 12; });
        if (!near.length){ kept.push(t); return; }
        var group = near.concat([t]);
        var keeper = group.filter(function(k){ return numOf(k.textContent) === 0; })[0] || group[0];
        group.forEach(function(k){
          if (k === keeper) return;
          k.style.display = 'none';
          k.setAttribute('data-hub-axis-hidden', '1');
          var idx = kept.indexOf(k);
          if (idx > -1) kept.splice(idx, 1);
        });
        if (kept.indexOf(keeper) < 0) kept.push(keeper);
      });

      /* 第二種疊字：負值柱的數值標籤會落在 x 軸標籤那一列上（例如 -0.8% 疊在「3个月」上）。
         這些柱值標籤改放到柱頂上方（若太靠頂就改放柱底下方）。 */
      var xRow = 0;
      Array.prototype.forEach.call(svg.querySelectorAll('text.bar-label'), function(t){
        xRow = Math.max(xRow, parseFloat(t.getAttribute('y')) || 0);
      });
      if (!xRow) return;
      var rects = Array.prototype.slice.call(svg.querySelectorAll('rect'));
      Array.prototype.forEach.call(svg.querySelectorAll('text.bar-value'), function(t){
        var y = parseFloat(t.getAttribute('y'));
        if (Math.abs(y - xRow) >= 15) return;
        var cx = parseFloat(t.getAttribute('x'));
        var best = null, bestD = 1e9;
        rects.forEach(function(r){
          var rx = parseFloat(r.getAttribute('x')) + parseFloat(r.getAttribute('width')) / 2;
          var d = Math.abs(rx - cx);
          if (d < bestD){ bestD = d; best = r; }
        });
        var ny = null;
        if (best && bestD < 40){
          var top = parseFloat(best.getAttribute('y'));
          ny = top - 4;
          if (ny < 12) ny = top + parseFloat(best.getAttribute('height')) + 10;
        } else {
          ny = y - 16;
        }
        t.setAttribute('y', ny.toFixed(1));
        t.setAttribute('data-hub-moved', '1');
      });
    });
  }
