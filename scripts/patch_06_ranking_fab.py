...
# 2) 移除既有的浮動按鈕（任何版本）
m = re.search(r'<a id="fund-ranking-fab".*?</a>', s, re.S)
if m:
    s = s[:m.start()] + s[m.end():]
    changed.append("移除舊浮動按鈕")

# 3) 注入新版浮動按鈕：基金月榜
FAB = ('<a id="fund-ranking-fab" href="./fund-ranking.html" '
...
# 4) CSS（若尚未有）
FAB_CSS = (
    "#fund-ranking-fab{position:fixed;right:18px;bottom:18px;z-index:80;"
    ...
    "#fund-ranking-fab:hover{border-color:var(--hub-red);color:var(--hub-red)}"
    "#fund-ranking-fab:active{transform:scale(.97)}"
    "@media (max-width:700px){#fund-ranking-fab{right:12px;bottom:12px;padding:8px 9px}}"
)
if "#fund-ranking-fab{" not in s:
    i = s.find("</style>")
    if i < 0:
